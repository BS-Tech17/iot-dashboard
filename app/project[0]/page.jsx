...
function ProjectPage({ params }) {
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-blue-400 via-white to-gray-800">
      <aside className="w-64 bg-gradient-to-br from-blue-400 via-blue-400 to-gray-800 text-white flex flex-col p-6 gap-4 border-r border-cyan-300">
        <div className="text-2xl font-bold mb-8 text-cyan-300">Project Menu</div>
        <Link href="/" className="hover:text-cyan-400">🏠 Home</Link>
        <Link href={`/projectSettings/${id}`} className="hover:text-white-400">⚙️ Project Settings</Link>
      ...
      <main className="flex-1 py-12 px-8 flex">
        ...
        <aside className="w-96 flex flex-col gap-4">
          <Card className="bg-gray-950 border-blue-400 p-6 py-10 text-white">
            <CardHeader>
              <CardTitle className="text-2xl text-blue-200">Sensor Status</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="bg-gray-950 border-cyan-300 p-6 text-center">
            <CardHeader>
              <CardTitle className="text-2xl text-blue-200">Actuators</CardTitle>
            </CardHeader>
            <CardContent>
              {activeActuators.length === 0 && !showSetup ? (
                <Button onClick={() => setShowSetup(true)} className="bg-cyan-700 hover:bg-cyan-600 text-white">
                  Set Up the Actuators
                </Button>
              ) : showSetup ? (
                <div className="space-y-4 text-white">
                  <Select value={selectedActuator} onValueChange={setSelectedActuator}>
                    <SelectTrigger className="w-full bg-gray-800 text-white border-cyan-300">
                  ...
                  <Button onClick={handleCreateActuator} className="w-full bg-cyan-700 hover:bg-cyan-600 text-white">
                    Create
                  </Button>
                </div>
              ) : (
